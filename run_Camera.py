...
def haarcascade_eye(img_src):
    face_cascade = cv2.CascadeClassifier(
        'D:/Anaconda3/envs/tensorflow/Lib/site-packages/cv2/data/haarcascade_frontalface_alt.xml')
    eye_cascade = cv2.CascadeClassifier('D:/Anaconda3/envs/tensorflow/Lib/site-packages/cv2/data/haarcascade_eye.xml')
    ...
    img_src = './resultPic/pic_' + str(now_time) + '.png'
...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label.setText(_translate("MainWindow", "人脸检测演示"))
    ...
    def takePhoto(self):
        if self.timer_camera.isActive() != False:
            now_time = time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))
            print(now_time)
            img_src = './picture/pic_'+str(now_time)+'.png'
            cv2.imwrite(img_src, self.image)



            cv2.putText(self.image, 'Picture saved !',

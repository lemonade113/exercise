...
def Chinese_plot_box(image,label,x,sizes,colour=None,line_thickness=None):
    ...
    font = ImageFont.truetype("./font/simhei.ttf",sizes, encoding="utf-8")
                    ...
                    for k, d in enumerate(faces):
                        ...
                        for i in range(68):
                            ...
                            cv2.putText(im_rd, str(i), (shape.part(i).x, shape.part(i).y), cv2.FONT_HERSHEY_SIMPLEX,
                                        0.5, (255, 255, 255))
                            # 演示时屏蔽脸部特征68个圆点干扰
                            # cv2.putText(im_rd, "", (shape.part(i).x, shape.part(i).y), cv2.FONT_HERSHEY_SIMPLEX,
                            #             0.5, (255, 255, 255))
                        ...
                        if round(mouth_height >= 0.03):
                            if eye_hight >= 0.056:
                                cv2.putText(im_rd, zh_ch("Amazing"), (d.left(), d.bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255), 2, 4)
                                Chinese_plot_box(im_rd, label='震惊', x=[0, 0, d.left(), d.bottom() + 20], sizes=20, colour=None,
                                                 line_thickness=None)
                            else:
                                cv2.putText(im_rd, zh_ch("Happy"), (d.left(), d.bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255), 2, 4)
                                Chinese_plot_box(im_rd, label='高兴', x=[523, 719, 641, 823], sizes=20, colour=None,
                                                 line_thickness=None)

                        # 没有张嘴，可能是正常和生气，通过眉毛区分
                        else:
                            if self.brow_k <= -0.3:
                                cv2.putText(im_rd, zh_ch("Angry"), (d.left(), d.bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255), 2, 4)
                                Chinese_plot_box(im_rd, label='生气', x=[523, 719, 641, 823], sizes=20, colour=None,
                                                 line_thickness=None)
                            else:
                                cv2.putText(im_rd, zh_ch("Nature"), (d.left(), d.bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255), 2, 4)
                                Chinese_plot_box(im_rd, label='自然', x=[523, 719, 641, 823], sizes=20, colour=None,
                                                 line_thickness=None)
                # 标出人脸数
                cv2.putText(im_rd, "Face-" + str(len(faces)), (20, 50), font, 0.6, (0, 0, 255), 1, cv2.LINE_AA)
            else:
                # 没有检测到人脸
                cv2.putText(im_rd, zh_ch("No Face"), (20, 50), font, 0.6, (0, 0, 255), 1, cv2.LINE_AA)
